...
{
    public static async Task<int> VerificarResponsavelLegalPorCPFAsync(
    ...
    MyDbContext context)
    {
        ...
        if (usuario == 0)
        {
            ...
            usuario = await AdicionarNovoResponsavelLegalAsync(
                dados,
                context
            );
        }
    ...
    private static async Task<int> AdicionarNovoResponsavelLegalAsync(
        Estabelecimento_Responsavel_LegalDto dados,
        MyDbContext context
    )
    {
    ...
    public static async Task<int> VerificarResponsavelTecnicoPorCPFAsync(
    Estabelecimento_Responsavel_TecnicoDto dados,
    MyDbContext context)
    {
        var usuario = await context.Usuario
            .Where(u => u.CPF == dados.CPF)
        ...
        if (usuario == 0)
        {
            ...
            usuario = await AdicionarNovoResponsavelTecnicoAsync(
                dados,
                context
            );
        }
    ...
    private static async Task<int> AdicionarNovoResponsavelTecnicoAsync(
        Estabelecimento_Responsavel_TecnicoDto dados,
        MyDbContext context
    )
    {
